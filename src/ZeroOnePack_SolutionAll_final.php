...
class ZeroOnePack_SolutionAll_final extends AbstractKnapsackSolution {
    public static function fillPack(array $items, KnapsackPack $pack, bool $fitPackVolume = false) {
        ...
        for ($i = 0; $i < $N; $i++) {
            ...
            self::fillItem($items[$i], $i, $V, $f, $g, $loop_count, $bound, $g_sum, $g_all);
        }
    ...
    public static function fillItem(KnapsackItem $item, $i, $V, &$f, &$g, &$loop_count, &...$reserves) {
        for ($v = 0; $v <= $V; $v++) {
            ...
            $reserves[2][$i+1][$v] = $reserves[2][$i][$v];
            $loop_count++;
        }
        ...
        for ($v = $V; $v >= $reserves[0]; $v--) {
            ...
            if ($f[$v] == $left && $left > $right) {
                ...
                $reserves[2][$i+1][$v] = $reserves[2][$i][$v-$item->getCost()];
                for($x = 0; $x<count($reserves[2][$i+1][$v]); $x++) $reserves[2][$i+1][$v][$x][] = $i;
            } else if ($f[$v] == $left && $left == $right) {
                $reserves[1][$i+1][$v] = $reserves[1][$i][$v-$item->getCost()] + $reserves[1][$i][$v];

                $reserves[2][$i+1][$v] = $reserves[2][$i][$v-$item->getCost()];
                for($x = 0; $x<count($reserves[2][$i+1][$v]); $x++) $reserves[2][$i+1][$v][$x][] = $i;
                $reserves[2][$i+1][$v] = array_merge($reserves[2][$i+1][$v], $reserves[2][$i][$v]);
            } else if ($f[$v] == $right) {
                $reserves[1][$i+1][$v] = $reserves[1][$i][$v];
                $reserves[2][$i+1][$v] = $reserves[2][$i][$v];
            }
